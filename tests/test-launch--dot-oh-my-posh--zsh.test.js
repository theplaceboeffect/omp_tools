...
test.describe('dot-oh-my-posh.zsh script tests', () => {
    let tempDir;
    let originalCwd;
    ...
    test('should execute without errors and return environment information', async () => {
        const scriptPath = path.join(originalCwd, 'dot-oh-my-posh.zsh');
    ...
    test('should handle environment detection correctly', async () => {
        ...
        // Should detect macOS correctly on this system
        expect(result).toContain('Operating System: macOS');
        expect(result).toContain('Shell: zsh');
        
        // Should attempt to detect package manager
        expect(result).toContain('Package Manager:');
    });
}); 